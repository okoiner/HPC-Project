...
plot_names = ["B_perf_PolyDecay", "B_perf_ExpDecay", "B_perf_MNIST", "B_perf_Year_a", "B_perf_Year_b"]
...
for file_id, file_name in enumerate(file_names):
	...
	for l_id, l in enumerate(ll):
		for sketch_id, sketch in enumerate(sketchs):
			...
			kk = rows["k"]
			plt.semilogy(kk, runtimes, line_style_dict[sketch_id], c = color_dict[l_id], label="$\ell = %d$, " % l + "\t" + sketch_dict[sketch])
