...
plot_names = ["G_parallel", "G_sequential"]
titles = ["Parallel", "Sequential"]
...
for file_id, file_name in enumerate(file_names):
	...
	fig = plt.figure()
	for l_id, l in enumerate(reversed(ll)):
		for sketch_id, sketch in enumerate(sketchs):
			...
			kk = rows["k"]
			plt.semilogy(kk, runtimes, line_style_dict[sketch_id], c = color_dict[len(ll) - l_id - 1], label="$\ell = %d$, " % l + "\t" + sketch_dict[sketch])
